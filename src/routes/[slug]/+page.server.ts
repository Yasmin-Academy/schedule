...
import { resolveTranslations } from '$lib/i18n/template';

export const load: PageServerLoad = async ({ params, platform, url, request }) => {
	...
	try {
		...
		const eventType = await db
			.prepare(
				'SELECT id, name, name_ar, host_name_ar, slug, duration_minutes as duration, description, is_active, cover_image, invite_calendar FROM event_types WHERE user_id = ? AND slug = ? AND is_active = 1'
			)
			.bind(user.id, slug)
			.first<{
				id: string;
		...
		// Parse user settings
		let userSettings: { timeFormat?: string; defaultInviteCalendar?: string; translations?: any } = {};
		try {
			userSettings = user.settings ? JSON.parse(user.settings) : {};
		} catch {}

		// Resolve booking UI translations.
		// Public booking page supports `?lang=ar`.
		// If no explicit `?lang`, prefer Arabic when the browser's Accept-Language prefers Arabic.
		const explicitLang = url.searchParams.get('lang');
		let lang: 'en' | 'ar' = explicitLang === 'ar' ? 'ar' : explicitLang === 'en' ? 'en' : 'en';
		if (!explicitLang) {
			const accept = request.headers.get('accept-language') || '';
			// Matches: "ar", "ar-EG", "ar;q=0.9,en;q=0.8", etc.
			if (/(^|,|\s)ar(-|;|,|\s|$)/i.test(accept)) lang = 'ar';
		}
		const overrides = (userSettings.translations && userSettings.translations[lang]) || {};
		const translations = resolveTranslations(lang, overrides);
