<script lang="ts">
	...
	import { detectTimezone, getTimezoneLabel, getTimezoneWithTimeLocalized } from '$lib/constants/timezones';
	...
	// Use the $page store so navigation + query-string changes update the UI.
	const data = $derived($page.data as PageData);

	// Simple public booking localization.
	// Priority: explicit ?lang=..., otherwise infer from loaded translations (if they contain Arabic),
	// otherwise server-selected default (based on Accept-Language), then 'en'.
	function looksArabic(text: string) {
		return /[\u0600-\u06FF]/.test(text || '');
	}

	const inferredFromTranslations = $derived(
		looksArabic(String((data.translations as any)?.selectDate || '') + ' ' + String((data.translations as any)?.chooseTimezone || ''))
			? 'ar'
			: 'en'
	);

	let lang = $state((data.lang as any) === 'ar' ? 'ar' : inferredFromTranslations);
	$effect(() => {
		const qp = (($page.url.searchParams.get('lang') || '') + '').toLowerCase();
		if (qp === 'ar' || qp === 'en') {
			lang = qp;
		} else {
			lang = (data.lang as any) === 'ar' ? 'ar' : inferredFromTranslations;
		}
		if (browser) {
	...
	const tr = $derived(data.translations);
	const t = (key: string, fallback: string) => (tr && (tr as any)[key]) || fallback;

	function minutesLabel(n: number) {
		// Keep it simple: Arabic uses 'دقيقة', English uses 'min'
		return lang === 'ar' ? `${nf.format(n)} دقيقة` : `${n} min`;
	}
	...
	// meetingType removed (Google/Outlook integrations removed)
	...
	async function fetchMonthAvailability() {
		...
		try {
			...
			const response = await fetch(
				`/api/availability/month?event=${data.slug}&month=${monthStr}&tz=${encodeURIComponent(selectedTimezone)}`
			);
	...
	async function handleDateSelect(dateStr: string) {
		...
		try {
			const response = await fetch(
				`/api/availability?event=${data.slug}&date=${dateStr}&tz=${encodeURIComponent(selectedTimezone)}`
			);
			if (!response.ok) throw new Error('Failed to fetch availability');
...
<svelte:head>
	<title>{data.eventType?.name || t('bookingTitle', 'Book a consultation')}</title>
