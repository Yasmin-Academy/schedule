...
export const actions: Actions = {
	save: async (event) => {
		...
		const hostNameAr = formData.get('host_name_ar');
		...
		const lengthError = validateFields([
			...
			validateLength((nameAr || '').toString(), 'Arabic name', MAX_LENGTHS.name, false),
			validateLength((hostNameAr || '').toString(), 'Arabic host name', MAX_LENGTHS.name, false),
		...
		try {
			...
			await db
				.prepare(
					`UPDATE event_types
					SET name = ?, name_ar = ?, host_name_ar = ?, slug = ?, duration_minutes = ?, description = ?, is_active = ?, cover_image = ?,
						availability_calendars = ?, invite_calendar = ?
					WHERE id = ? AND user_id = ?`
				)
				.bind(
					name.toString(),
					(nameAr || '').toString().trim() || null,
					(hostNameAr || '').toString().trim() || null,
					slugStr,
					parseInt(duration.toString()),
					description.toString(),
					isActive ? 1 : 0,
					coverImage ? coverImage.toString() : null,
					// When override_calendar_settings is not enabled, store NULLs to use global settings.
	...
	delete: async (event) => {
		...
		const existing = await db
			...
			.bind(id, userId)
			.first();
		if (!existing) return fail(404, { error: 'Event type not found' });

		// Remove dependent rows that are not covered by FK cascade.
		await db.prepare('DELETE FROM bookings WHERE event_type_id = ?').bind(id).run();
		await db.prepare('DELETE FROM event_types WHERE id = ? AND user_id = ?').bind(id, userId).run();
