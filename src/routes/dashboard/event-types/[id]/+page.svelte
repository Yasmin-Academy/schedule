<script lang="ts">
	...
	let nameAr = $state(data.eventType.name_ar || '');
	let hostNameAr = $state(data.eventType.host_name_ar || '');
				...
				<div class="space-y-6">
					...
					<div>
						...
						<input
							...
							bind:value={nameAr}
							dir="rtl"
							placeholder="مثال: اجتماع 30 دقيقة"
							class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
						/>
						<p class="text-xs text-gray-500 mt-1">
							Shown on the booking page when the visitor uses Arabic (e.g., ?lang=ar).
						</p>
					</div>

					<!-- Host Name (Arabic) -->
					<div>
						<label for="host_name_ar" class="block text-sm font-medium text-gray-700 mb-2">
							Host Name (Arabic)
						</label>
						<input
							type="text"
							id="host_name_ar"
							name="host_name_ar"
							bind:value={hostNameAr}
							dir="rtl"
							placeholder="مثال: سارة"
							class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
						/>
						<p class="text-xs text-gray-500 mt-1">
							Optional override for the host name displayed on the booking page in Arabic.
						</p>
					</div>
					...
					<div class="flex gap-4 pt-4">
						...
						<!-- Note: can't nest forms; use button-level formaction to target the delete action. -->
						<button
							type="submit"
							formmethod="POST"
							formaction="?/delete"
							...
							onclick={(e) => {
								if (!confirm('Delete this event type? This will also delete its bookings.')) e.preventDefault();
							}}
						>
							Delete
						</button>
