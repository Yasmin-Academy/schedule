...
export const PUT: RequestHandler = async (event) => {
	...
	try {
		const body = await event.request.json() as {
			...
			caldav?: { calendarUrl?: string; username?: string; password?: string };
		};
		const { name, profileImage, brandColor, contactEmail, timeFormat, widgetTranslations, translations, caldav } = body;
		...
		// If this is a settings-only update (no name provided)
		if (name === undefined && (caldav !== undefined || timeFormat !== undefined || widgetTranslations !== undefined || translations !== undefined)) {
			let nextCaldav = (existingSettings as any).caldav || {};
			if (caldav !== undefined) {
				nextCaldav = {
					...nextCaldav,
					...(caldav.calendarUrl !== undefined && { calendarUrl: caldav.calendarUrl }),
					...(caldav.username !== undefined && { username: caldav.username }),
					// Only overwrite password if provided (non-empty)
					...(caldav.password ? { password: caldav.password } : {})
				};
			}
			...
			const newSettings = {
				...
				...(widgetTranslations !== undefined && { widgetTranslations }),
			...(translations !== undefined && { translations }),
				...(translations !== undefined && { translations }),
				...(caldav !== undefined && { caldav: nextCaldav })
			};
