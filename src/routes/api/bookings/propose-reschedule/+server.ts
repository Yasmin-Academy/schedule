			...
			try {
				...
				await sendRescheduleProposalEmail(
					...
					},
					{
						...
						replyTo: env.MAILGUN_REPLY_TO || booking.contact_email || booking.host_email
					}
				);
			} catch (emailErr) {
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f3f4f6;">
	...
	const subject = `Reschedule Request: ${data.eventName} with ${data.hostName}`;
...
body.set('from', normalizeFrom(config.from, data.hostName));
body.set('to', data.attendeeEmail);
