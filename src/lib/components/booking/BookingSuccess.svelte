<script lang="ts">
	...
	const meetingLabel = $derived(tr.openOnlineRoom ?? (lang === 'ar' ? 'افتح الغرفة الأونلاين' : 'Open Online Room'));
</script>
...
<div class="bg-white rounded-2xl shadow-lg p-6 sm:p-8 max-w-md w-full mx-2">
	<div class="text-center">
		...
		<h1 class="text-xl sm:text-2xl font-semibold text-gray-900 mb-2">
			{tr.youAreScheduled ?? (lang === 'ar' ? 'تم تأكيد الحجز' : 'You are scheduled')}
		</h1>
		<p class="text-gray-600 mb-6 sm:mb-8 text-sm sm:text-base">
			{tr.inviteSent ?? (lang === 'ar' ? 'تم إرسال دعوة تقويم إلى بريدك الإلكتروني.' : 'A calendar invitation has been sent to your email address.')}
		</p>
