...
<div class="timezone-selector fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100%-2rem)] max-w-md foreground-border rounded-xl shadow-2xl z-50 max-h-[80vh] flex flex-col overflow-hidden">
	<!-- Search -->
	<div class="p-3 border-b foreground-border">
		<div class="relative">
			<svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				class="input w-full pl-10 pr-4 py-2 text-sm foreground-border rounded-lg focus:ring-2 focus:border-transparent outline-none"
	...
	<!-- Header -->
		<div class="px-4 py-2 border-b foreground-border background-secondary">
	...
	<!-- Timezone list -->
		...
		{#each Object.entries(filteredGroups()) as [group, zones]}
			<div role="group" aria-labelledby="group-{group}">
				<div id="group-{group}" class="px-4 py-2 text-xs font-semibold text-muted uppercase tracking-wide background-secondary sticky top-0">
				...
				{#each zones as tz}
					...
					<button
						...
						class="w-full px-4 py-2.5 flex items-center justify-between text-left text-sm hover:background-secondary transition-colors {isSelected ? 'font-medium' : 'text-primary'}"
					...
					>
						...
						<span class="text-muted tabular-nums">{getCurrentTime(tz.value, false, locale())}</span>
					</button>
				{/each}
			</div>
		{/each}
