<script lang="ts">
	...
	let widgetTranslationsText = $state(JSON.stringify(getUserSettings().widgetTranslations || {}, null, 2));

	// Keep this as a plain JS string (no escaping for HTML attributes). We'll bind it via
	// `placeholder={...}` so Svelte doesn't try to parse backslashes inside markup.
	const widgetTranslationsPlaceholder =
		'{"onlineRoom":"Online Room","roomNote":"Room link and details will be sent by email after booking."}';
	...
	async function saveProfile() {
		...
		try {
			let widgetTranslations: Record<string, string> | undefined = undefined;
			try {
				const parsed = JSON.parse(widgetTranslationsText || '{}');
				if (parsed && typeof parsed === 'object') widgetTranslations = parsed;
			} catch {
				throw new Error('Widget translations must be valid JSON');
			}
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
	...
	{#if showProfileEdit}
		...
		<div class="space-y-4">
			...
			<!-- Widget Translations -->
			<div class="mt-6">
				<label class="block text-sm font-medium text-gray-700 mb-3" for="widgetTranslations">
					Widget Translations (JSON)
				</label>
					<textarea
					id="widgetTranslations"
					class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm font-mono"
					rows="6"
						placeholder={`Paste JSON, e.g. ${widgetTranslationsPlaceholder}`}
					bind:value={widgetTranslationsText}
				></textarea>
				<p class="text-xs text-gray-500 mt-2">Optional. Keys are merged into the widget language defaults.</p>
			</div>
