...
export function generateBookingEmail(data: BookingEmailData): string {
	...
	const meetingLabel = 'Open online room';
...
${data.meetingUrl ? `Online Room: ${data.meetingUrl}` : 'Online Room link will be sent by email.'}


${data.meetingUrl ? `Open online room: ${data.meetingUrl}` : ''}
